...
class DirectoryGraph {
    ...
    vector<string> iterativeBFS( adjList adj ) {
        ...
        while( !q.empty() ) {
            ...
            int pos = this->search( tempID ) ;
            q.pop() ;
            adjListNode *cur = adjL.at( this->search( tempID ) ).head ;



            if ( adjL.at( this->search( tempID ) ).infList.size() != 0 ) {
                // sort( visited.begin(), visited.end(), less<string>() ) ;


                for ( int i = 0 ; i<adjL.at( pos ).infList.size() ; ++i ) {
                    if ( find( visited.begin(), visited.end(), adjL.at( this->search( tempID ) ).infList.at(i) ) == visited.end() )
                        visited.push_back( adjL.at( this->search( tempID ) ).infList.at(i) ) ;
                } // for
...
public:
		...
		void Two( ) {
            if ( List.GetSize() != 0 ) {
                ...
                cout << "Cost time: " << (end-begin)/CLOCKS_PER_SEC << endl ;
